...
export class UserRepository {
  ...
  async getUserByTelegramId({ telegramId, tx }: { telegramId: string; tx?: TransactionType }) {
    ...
    try {
      ...
      console.error("getUserByTelegramId error", e);
      return null;
    }
  }
  ...
  async createUser({ user, tx, referer_id }: { user: CreateUserType; referer_id?: string; tx?: TransactionType }) {
    const db_client = tx ? tx : dataBase;
    const start_today = getStartOfToday();
    try {
      const new_user = await db_client.user.create({
        data: {
          ...
          profile: {
            create: {
              last_charge_recovery: start_today,
              last_qi_update: start_today,
              last_fight_time: start_today,
              last_hp_update: start_today,
            },
          },
          ...
          mine: {
            create: {
              last_mine_at: start_today,
              last_energy_at: start_today,
            },
          },
          spirit_path: { create: {} },
        },
  ...
  async updateUser({ user, tx }: { user: CreateUserType; tx?: TransactionType }) {
    const db_client = tx ? tx : dataBase;
    try {
